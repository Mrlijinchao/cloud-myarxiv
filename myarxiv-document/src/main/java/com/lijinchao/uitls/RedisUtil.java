...
public final class RedisUtil {
    ...
    public Object getByTokenNew(String token) {
        Set<String> keys = redisTemplate.keys(token+":*");
        if(!CollectionUtils.isEmpty(keys)){
//            List<Object> objects = redisTemplate.opsForValue().multiGet(keys);
//            return objects.get(0);
     ...
     * @return true成功 false 失败
     */
    public boolean updateById(String id, Object value, long time, TimeUnit timeUnit) {
        try {
            Set<String> keys = redisTemplate.keys("*:"+id);
            if (time > 0) {
                if(CollectionUtils.isEmpty(keys)){
     ...
     * @return true成功 false 失败
     */
    public boolean updateByToken(String token, Object value, long time, TimeUnit timeUnit) {
        try {
            Set<String> keys = redisTemplate.keys(token+":*");
            if (time > 0) {
                if(CollectionUtils.isEmpty(keys)){
     ...
     * @return true 存在 false不存在
     */
    public boolean hasKeyByToken(String token) {
        try {
            Set<String> keys = redisTemplate.keys(token+":*");
            if(!CollectionUtils.isEmpty(keys)){
                return true;
